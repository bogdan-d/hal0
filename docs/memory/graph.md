...
hal0's [memory engine](./index.md) (Cognee) can build a **knowledge
...
Graph extraction is **off by default** in v0.3. See [ADR-0014](../../internal/adr/0014-cognee-graph-extraction-model-gate.md) for the
...
(route × model). That's a v0.4 deliverable.
...
Save flips the live wrapper — no restart needed.
...
rejects before sending; the server-side validator (`config.memory_graph_invalid`) also enforces it.
...
- [`memory_search` MCP tool](../api/mcp.md#memory_search)
