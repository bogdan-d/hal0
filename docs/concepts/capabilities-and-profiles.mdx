...
description: How hal0 separates what a slot does (capabilities) from how it runs (profiles) and where it runs (device) — plus the device-to-profile defaults and the NPU trio.
sidebar:
  order: 30
---
...
|---|---|---|
| `rocm` | GPU (ROCm) | MoE agents |
| `rocm-mtp` | GPU (ROCm) | Dense chat with multi-token-prediction speculative decoding |
...
— `stt-npu` and `embed-npu` — whose requests are posted straight to the same
