...
description: Watch hal0 run — tail per-unit journald logs, stream the API log tail with priority filters, and read the durable Activity and Audit log of config changes and slot state transitions.
sidebar:
  order: 120
---
...
| `level`     | —       | Filter to this priority **and higher severity**: `warning` returns warning + error + critical and up. |
