...
description: Route requests to external OpenAI-compatible LLM providers (OpenAI, Anthropic, OpenRouter, Ollama, custom) by editing upstreams.toml, then write the API key and test the connection.
sidebar:
  order: 40
---
...
4. **`auth_style`** — one of `bearer`, `header`, or `none`.
...
This probes the upstream's `/v1/models` and returns a reachability report:
