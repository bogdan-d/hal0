...
description: Complete reference for the hal0 command-line tool — every top-level command and sub-app (slot, model, registry, memory, config, doctor, capabilities, agent, migrate).
sidebar:
  order: 10
---
...
- `--port` / `-p` — host port; auto-assigned from the free range `8081–8099`
  when omitted.
- `--ctx-size` — context window in tokens (default `4096`, minimum `128`).
