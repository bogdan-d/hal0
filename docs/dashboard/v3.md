...
- Loaded-slot grid: per-slot state, model, port, KV% (Vulkan slots
  derive KV% locally — see [container runtime](/docs/operate/container-runtime/)).
...
- **Slot indicator dots** (PR #314) — small per-message dot showing
  ...
  [slots, profiles, and the catalog](/docs/operate/container-runtime/#slots-profiles-and-the-catalog)).
...
- [Container runtime](/docs/operate/container-runtime/) — the per-slot
  podman units every inference panel scrapes.
