...
- `tools.gated` calls go through the [ADR-0004 approval queue](./agents.md#approval-queue).
- `tools.blocked` is a hard reject the dashboard can't undo
  (installer-pinned blocks survive user edits; TOML edit always wins,
  loudly).
- Tokens never live in TOML — `auth.kind = "bearer-from-env"` points
  at an env var the agent driver loads at process startup.
...
`ValidationError` with the offending tool name in the message. Tests
in `tests/config/test_agent_schema.py` pin this.
