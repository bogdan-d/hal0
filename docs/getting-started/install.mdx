...
install the Main slot is seeded but `offline` until `hal0 setup`
(or the CLI) loads a model into it.
...
flag, then `HAL0_MODELS_DIR`, then `/var/lib/hal0/models` (default —
no interactive prompt; model selection happens via `hal0 setup`).
The path is persisted as
`[models].pull_root` and auto-included in `[models].roots` so a fresh
install scans the existing tree on first boot.

## Authentication & HTTPS

hal0 ships with **no built-in auth and no bundled TLS**. `hal0-api`
binds `0.0.0.0:8080` open on the LAN and treats every request as
authenticated by virtue of network reachability — the right posture
for a homelab appliance on a trusted network
([ADR-0012](https://github.com/Hal0ai/hal0/blob/main/docs/internal/adr/0012-remove-auth-and-caddy.md)
removed the old built-in auth and the bundled Caddy edge).
...
hostname — front it with **your own reverse proxy** (Traefik, Caddy,
nginx, or a Cloudflare Tunnel) that owns authentication and HTTPS. See
[Authentication & Security](/docs/operate/auth/) for worked Traefik,
nginx, and Cloudflare Tunnel examples.
...
the built-in slots `offline`; run
[`hal0 setup`](/docs/getting-started/first-model/) to load models and
bring them up.
...
<CardGrid>
  <LinkCard
    ...
    description="Run hal0 setup to pick models, enable Extensions, and start your first slot." />
  <LinkCard
