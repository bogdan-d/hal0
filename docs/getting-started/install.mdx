---
title: Install hal0
description: Install hal0 with one command — the bootstrap fetches a signed release, verifies it with SHA-256 and cosign, installs podman, and brings up the API.
sidebar:
  order: 10
---
...
4. **Unpack and hand off** to `installer/install.sh`, which runs
   ...
   dashboard, writes config and systemd units, runs the hardware probe,
   and starts the API.
...
| `HAL0_MODELS_DIR` | `<var-lib>/models` | Where Hugging Face pulls land |
| `HAL0_PYTHON` | `python3` | Python interpreter to build the venv from |
| `HAL0_NO_PROBE` | _(unset)_ | Set to `1` to skip the post-install hardware probe |
...
<Aside type="tip">
The model pull directory must be an **absolute** path. When you run the
installer on a TTY without specifying one, it prompts interactively and
defaults to `<var-lib>/models`.
</Aside>
...
data directories, the `hal0` system user and group, all hal0/toolbox
podman images, and the FastFlowLM package. It prompts for a typed
`DELETE` confirmation unless you pass `--force` (or set `HAL0_FORCE=1`).
