...
- **Privileged LXC on Proxmox (recommended for homelabs).** GPU and
  ...
  the one shape that's happy unprivileged. The unified-memory bar in
  the dashboard surfaces the LXC's cgroup slice and, when you wire in
  a `PVEAuditor` token, the physical host total plus other tenants.
  That's the homelab payoff: hal0 knows it isn't the only thing on
  the box.
...
<img
  ...
  alt="Unified-memory bar with segments labelled GTT · inference, System RAM, Proxmox host, and Free."
  width="1172"
  height="114"
/>
