...
<Aside type="note">
  ...
  required** — there is no built-in auth and no bundled TLS. That's the
  trusted-LAN posture. For remote access or anything internet-facing,
  front hal0-api with **your own reverse proxy** (Traefik, Caddy,
  nginx, or a Cloudflare Tunnel) that owns HTTPS and authentication.
  See [Authentication & Security](/docs/operate/auth/) for worked
  examples.
</Aside>
